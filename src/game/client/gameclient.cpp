...
{
	{
		HandleBot(m_BotInput);
		mem_copy(pData, &m_BotInput, sizeof(m_BotInput));
		return sizeof(m_BotInput);
	}
...
void CGameClient::HandleBot(CNetObj_PlayerInput & Input)
{
	...
	m_pCharacter = nullptr;
		...
		CCharacter *p = (CCharacter *)m_GameWorld.FindFirst(CGameWorld::ENTTYPE_CHARACTER);
		for(; p; p = (CCharacter *)p->TypeNext())
		{
			if(p->GetCid() == m_GameWorld.m_LocalClientId)
			{
				m_pCharacter = p;
				break;
			}
	...
	
	{
		...
		CPickup *p = (CPickup *)m_GameWorld.FindFirst(CGameWorld::ENTTYPE_PICKUP);
	...
	
	{
		...
		for(; p; p = (CCharacter *)p->TypeNext())
		{
			...
			if(IsTeamPlay() && m_aClients[GetCharacter()->GetCid()].m_Team == m_aClients[p->GetCid()].m_Team)
				continue;
	...
	if(pClosestChar)
	{
		...
		Input.m_TargetX = m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos.x - m_pPos->x; // aim
		Input.m_TargetY = m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos.y - m_pPos->y;
		if(!Collision()->FastIntersectLine(*m_pPos,m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos,nullptr,nullptr) && !m_firedelay)
			m_firedelay = m_GameWorld.GameTick() + 50;
		else
			m_firedelay = 0;
		...
		if((m_pCore->m_ActiveWeapon == WEAPON_LASER ? (!Collision()->FastIntersectLine(*m_pPos,m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos,nullptr,nullptr) && distance(*m_pPos, m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos) < m_aTuning[0].m_LaserReach + 28.f) : !Collision()->FastIntersectLine(*m_pPos,m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos,nullptr,nullptr)) || m_pCore->m_aWeapons[WEAPON_NINJA].m_Got)
		{
			...
			if(m_GameWorld.GameTick() % m_GameWorld.GameTickSpeed() == 0)
			{
				Input.m_Hook = false;
			}
			else if(distance(*m_pPos, m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos) < m_aTuning[0].m_HookLength)
			{
				Input.m_Hook = true;
			}
		}
		
		if(Collision()->FastIntersectLine(*m_pPos,m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos,nullptr,nullptr) && m_pCore->m_ActiveWeapon == WEAPON_LASER)
		{
			bool fire = false;
			
			float halfy = (m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos.y - m_pPos->y) / 2;
			float halfx = (m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos.x - m_pPos->x) / 2;
			...
			//try y first
			if(Collision()->FastIntersectLine(vec2(m_pPos->x + m_aTuning[0].m_LaserReach / 2.f,halfy + m_pPos->y),vec2(m_pPos->x,halfy + m_pPos->y),nullptr,&tempvar))
			{
				Collision()->UnIntersectLineKZ(vec2(m_pPos->x + m_aTuning[0].m_LaserReach / 2.f,halfy + m_pPos->y),vec2(m_pPos->x,halfy + m_pPos->y),&BouncePos_Right,nullptr);
				if(!Collision()->IntersectLine(*m_pPos,BouncePos_Right,nullptr,nullptr) && tempvar.x > BouncePos_Right.x)
					Bounced_Right = true;
			}
			if(Collision()->FastIntersectLine(vec2(m_pPos->x - m_aTuning[0].m_LaserReach / 2.f,halfy + m_pPos->y),vec2(m_pPos->x,halfy + m_pPos->y),nullptr,&tempvar))
			{
				Collision()->UnIntersectLineKZ(vec2(m_pPos->x - m_aTuning[0].m_LaserReach / 2.f,halfy + m_pPos->y),vec2(m_pPos->x,halfy + m_pPos->y),&BouncePos_Left,nullptr);
				if(!Collision()->IntersectLine(*m_pPos,BouncePos_Left,nullptr,nullptr) && tempvar.x < BouncePos_Left.x)
					Bounced_Left = true;
			}
			//try x now
			if(Collision()->FastIntersectLine(vec2(halfx + m_pPos->x,m_pPos->y + m_aTuning[0].m_LaserReach / 2.f),vec2(halfx + m_pPos->x,m_pPos->y),nullptr,&tempvar))
			{
				Collision()->UnIntersectLineKZ(vec2(halfx + m_pPos->x,m_pPos->y + m_aTuning[0].m_LaserReach / 2.f),vec2(halfx + m_pPos->x,m_pPos->y),&BouncePos_Down,nullptr);
				if(!Collision()->IntersectLine(*m_pPos,BouncePos_Down,nullptr,nullptr) && tempvar.y > BouncePos_Down.y)
					Bounced_Down = true;
			}
			if(Collision()->FastIntersectLine(vec2(halfx + m_pPos->x,m_pPos->y - m_aTuning[0].m_LaserReach / 2.f),vec2(halfx + m_pPos->x,m_pPos->y),nullptr,&BouncePos_Up))
			{
				Collision()->UnIntersectLineKZ(vec2(halfx + m_pPos->x,m_pPos->y - m_aTuning[0].m_LaserReach / 2.f),vec2(halfx + m_pPos->x,m_pPos->y),&BouncePos_Up,nullptr);
				if(!Collision()->IntersectLine(*m_pPos,BouncePos_Up,nullptr,nullptr) && tempvar.y < BouncePos_Up.y)
					Bounced_Up = true;
			}
			
			//try y
			if(Bounced_Right && (BouncePos_Right.y > (halfy + m_pPos->y) - 5.f && BouncePos_Right.y < (halfy + m_pPos->y) + 5.f) && !Collision()->FastIntersectLine(BouncePos_Right,m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos,nullptr,nullptr) && m_GameWorld.IntersectCharacter(BouncePos_Right, vec2(m_pPos->x,m_pPos->y + (BouncePos_Right.y - m_pPos->y)*2), 0.f, At, GetCharacter()))
			{
				Input.m_TargetX = BouncePos_Right.x - m_pPos->x; // aim
				Input.m_TargetY = BouncePos_Right.y - m_pPos->y;
				fire = true;
			}
			else if(Bounced_Left && (BouncePos_Left.y > (halfy + m_pPos->y) - 5.f && BouncePos_Left.y < (halfy + m_pPos->y) + 5.f) && !Collision()->FastIntersectLine(BouncePos_Left,m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos,nullptr,nullptr) && m_GameWorld.IntersectCharacter(BouncePos_Left, vec2(m_pPos->x,m_pPos->y + (BouncePos_Left.y - m_pPos->y)*2), 0.f, At, GetCharacter()))
			{
			...
			//try x
			if(Bounced_Up && (BouncePos_Up.x > (halfx + m_pPos->x) - 5.f && BouncePos_Up.x < (halfx + m_pPos->x) + 5.f) && !Collision()->FastIntersectLine(BouncePos_Up,m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos,nullptr,nullptr) && m_GameWorld.IntersectCharacter(BouncePos_Up, vec2(m_pPos->x + (BouncePos_Up.x - m_pPos->x)*2,m_pPos->y), 0.f, At, GetCharacter()))
			{
				Input.m_TargetX = BouncePos_Up.x - m_pPos->x; // aim
				Input.m_TargetY = BouncePos_Up.y - m_pPos->y;
				fire = true;
			}
			else if(Bounced_Down && (BouncePos_Down.x > (halfx + m_pPos->x) - 5.f && BouncePos_Down.x < (halfx + m_pPos->x) + 5.f) && !Collision()->FastIntersectLine(BouncePos_Down,m_aClients[pClosestChar->GetCid()].m_Predicted.m_Pos,nullptr,nullptr) && m_GameWorld.IntersectCharacter(BouncePos_Down, vec2(m_pPos->x + (BouncePos_Down.x - m_pPos->x)*2,m_pPos->y), 0.f, At, GetCharacter()))
			{
	...
	if(m_StopUntilTouchGround && GetCharacter()->IsGrounded())
	{
		...
		m_DontDoSmartTargetChase = m_GameWorld.GameTickSpeed() * 2;
	}
	...
	{
		if((GetCharacter()->IsGrounded() || (m_pCore->m_Jumps > 0 && m_pCore->m_Vel.y > 0)) && !Input.m_Jump)
			Input.m_Jump = true;
		else
			Input.m_Jump = false;
	}

	if(m_pCore->m_HookState == HOOK_GRABBED && m_pCore->HookedPlayer() == -1)
	{
		Input.m_Hook = 0;
	}

	if(Input.m_Fire | 1 && m_firedelay > m_GameWorld.GameTick())
	{
		Input.m_Fire = 0;
	}
